...
def generate_swap_type(method = "random", past_swaps = None, iter = None, past_times = None,
    ...
    if method == "random":
        ...
        if iter <= p:
            ...
            logit_probablity = np.sum(np.array(transformed_series)*np.flipud(AR_parameters)*0)
            ...
            if method == "AR_exog":
                ...
                past_times_prob_componet = np.sum((np.array(past_times))*np.flipud(AR_exog_params)*0.01)
...
def run_simulation(steps = 1000, x_balance = 1000000, y_balance = 10000):
    ...
    for i in range(steps):
        ...
        is_buy = generate_swap_type(method = "AR", past_swaps=swap_types, iter=i, AR_parameters=AR_parameter, past_times = times, AR_exog_params = AR_exog_params)
